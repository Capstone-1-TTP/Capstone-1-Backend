...
const seed = async () => {
  try {
    ...
    const ballots= await Ballot.bulkCreate([
      {ballotId: ballots[0].id, userId: users[0].id, pollId: polls[0].id, optionsRanking: [], isSubmitted: true},
      {ballotId: ballots[1].id, userId: users[1].id, pollId: polls[1].id, optionsRanking: [], isSubmitted: false},
      {ballotId: ballots[2].id, userId: users[2].id, pollId: polls[2].id, optionsRanking: [], isSubmitted: true}
    ]);
